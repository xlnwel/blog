...
$$
\max_{p,q}\mathcal J(p, q)=\sum_{t=1}^T\gamma^{t-1}\mathbb E_{s_{t}, a_{t}\sim q}\left[r(s_t,a_t)-\tau\log{q(a_t|s_t)\over p(a_t)}\right]\tag{1}
$$
...
=&\sum_{t=1}^T\gamma^{t-1}\mathbb E_{s_{t}, a_{t}\sim q}\left[r(s_t,a_t)\right]-\tau\sum_sq(s)D_{KL}(q(a|s)\Vert p(a))\tag{2}\\\
\overset{\mathrm{if}\ p(a)=q(a)}{=}&\sum_{t=1}^T\gamma^{t-1}\mathbb E_{s_{t}, a_{t}\sim q}\left[r(s_t,a_t)\right]-\tau I_{q}(S;A)\tag{3}\\\
where\quad q(s)=&{1\over Z}\sum_{t=1}^T\gamma^{t-1}\sum_{s_t}q(s_t)\tag{4}\\\
...
Here, $$q(s)$$ is the discounted marginal distribution over states. We kind of abuse the equality sign in Equation $$(2)$$ as we implicitly add the partition function $$1\over Z$$ to $$q(s)$$. This, however, does not change our optimization objective since it's a constant and can easily be corrected through $$\tau$$. 

From Equation $$(2)$$,  we can see that maximizing $$\mathcal J(p,q)$$ w.r.t. $$p$$ equally minimizes $$\sum_sq(s)D_{KL}(q(a\vert s)\Vert p(a))$$, which is minimized when $$p(a)=q(a)$$:
...
$$
\begin{align}
L(\theta)&:=E_{s,a,r,s'\sim replay}\left[\left((\mathcal T_{soft}^pQ)(s,a,s';\theta^-)-Q(s,a;\theta)\right)^2\right]\tag {13}\\\
...
Back when we discussed Eq.$$(3)$$, we said that 
