...
This method is not useful in large-scale problems, since computing $$ H^{-1} $$ is prohibitively costly (both in computation and memory). Instead, we generally solve $$ (9) $$ approximately using the [conjugate gradient method]({{ site.baseurl }}{% post_url 2018-08-27-CG %}) in which we directly compute $$Hx$$ by
