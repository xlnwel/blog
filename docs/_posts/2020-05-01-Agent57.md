...
However, in practice, [Badia et al. 2020](#ref1) find choosing an identity(Equation $$\eqref{eq:1}$$) or an $$h$$-transform(Equation $$\eqref{eq:2}$$) does not make any difference in terms of performance. Therefore, they simply use Equation $$\eqref{eq:1}$$, but still use the transformed Retrace loss functions.
