...
Therefore, $$\pmb b_j$$ is an orthonormal eigenvector of $$\pmb C$$ and $$\lambda_j$$ is its eigenvalue. This indicates that Equation $$\eqref{eq:4}$$ is minimized when $$\lambda_j$$ is the smallest $$n-k$$ eigenvalues of $$\pmb C$$. As a result, we have $$\bar{\pmb X}=\pmb X\pmb V_k$$, where the columns of $$\pmb V_k$$ are the eigenvectors of $$\pmb C$$ with the largest $$k$$ eigenvalues. 
