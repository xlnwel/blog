...
with \\(\tilde r\\) being the dual optimum. Because of the strictly concavity of \\(\bar{\mathcal H}\\), the primal optimum is obtained when the constraint is satisfied, i.e., \\(\rho_{\tilde \pi}=\rho_{E}\\)
