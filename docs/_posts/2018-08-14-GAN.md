...
1. The distribution of \\( z \\) used in test time must be the same as that in training time, since the generator mainly tries to approximate \\( P(x|z) \\) to \\( P(x) \\), where \\( P(x|z) \\) is represented by the generation network and \\( P(x) \\) is the real data distribution
