...
where \\([z_t,0]^\top\\) and \\([0,z_t]^\top\\) are of shape \\([1, 2\times|z|]\\) with \\(z_t\\) in the beginning/end. Thus, \\(v_t^w[z_t,0]^\top\\) writes \\(z_t\\) to the first half of a chosen row; and \\(v_t^{ret}[0,z_t]^\top\\) adds \\((1-\gamma)z_t\\) to all previously written rows.
...
This vector is then propagated through an MLP that produces an output \\(f^{post}(n_t)\\) of size \\(2\times |z|\\). The output of this MLP is added to the prior distribution to determine the posterior distribution: \\([\mu_t^{post},\log\Sigma_t^{post}]=f^{post}(n_t)+[\mu_t^{prior},\log\Sigma_t^{prior}]\\).
