...
A potential advantage of this simple product is that it introduces permutation invariant representation \\(q_\phi(z|c)\\). 
...
We optimize the inference network \\(q_\phi(z|c)\\) through the variational lower bound:
...
3. The inference network \\(q_\phi(z|c)\\) is trained using gradients from the Bellman update of the \\(Q\\)-network as we stated before. 
