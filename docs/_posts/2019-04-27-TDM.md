...
$$
\begin{align}
a_t=&\underset{a_{t:t+T}}{\arg\max}\sum_{i=t}^{t+T} r_c(s_i,a_i)\tag {2}\\\
...
Now assume we only care about the reward at every \\(K\\) step. In that case, Eq.\\((2)\\) equals to

$$
\begin{align}
a_t=&\underset{a_{t:K:t+T},s_{t+K:K:t+T}}{\arg\max}\sum_{i=t,t+K,\dots,t+T} r_c(s_i,a_i)\tag {3}\\\
...
$$
\begin{align}
a_t=&\underset{a_t,s_{t+T},a_{t+T}}{\arg\max}r_c(s_{t+T},a_{t+T})\tag{4}\\\
...
$$
\begin{align}
a_t=\underset{a}{\arg\max}Q(s_t,a,s_g,T)\tag {6}
\end{align}
$$
