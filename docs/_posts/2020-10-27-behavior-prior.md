...
$$
\mathcal L=\sum_{t}\mathbb E_{ \pi}\left[\gamma^tr(s_t,a_t)-\gamma^tD_{KL}(\pi(a_t|x_t)\Vert \pi_0(a_t|x_t^D))\right]\tag 8
$$
...
Because $$I(x_t^G,a_t\vert x_t^D)\le \mathbb E_\pi[D_{KL}(\pi(a_t\vert x_t)\Vert \pi_0(a_t\vert x_t^D))]$$, Equation $$(9)$$ is an upper bound of Equation $$(8)$$. 

The intuition behind Equation $$(9)$$ is that the agent should exhibit similar behaviors in different context where $$x_t^D$$ is shared across, and only need to adjust its behavior when the benefit of doing so outweighs the cost for processing information contained in $$x_t^G$$.
...
$$
\begin{align}
\pi_0(\tau)=\int\pi_0(\tau|y)\pi_0(y)dy\tag 9\\\
...
Unfortunately, it's difficult to directly compute the KL divergence between two complex distributions outlined in Equations $$(9)$$ and $$(10)$$. Instead, if we divide $$\pi$$ into higher level $$\pi^H(z_t\vert x_t)$$ and lower level $$\pi^L(a_t\vert z_t,x_t)$$ components, we can derive the following bound for the KL term
...
$$
\mathcal L=\sum_{t}\mathbb E_{ \pi}\left[\gamma^tr(s_t,a_t)-\gamma^tD_{KL}(\pi(z_t|x_t)\Vert \pi_0(z_t|x_t))\right]\tag{12}
$$

If we don't consider the concept of hierarchy, Equation $$(12)$$ simply lifts the KL penalty from the policy to a latent variable inside the neural network. 
