...
where \\(\mathbb E_{\pi}[-\log\pi(a|s)]\\) computes the policy entropy and \\(\mathcal H\\) is a desired expected entropy—Haarnoja&Zhou et al. choose the negative of the dimension of the action space as \\(\mathcal H\\). To solve this constraint optimization problem, we reformulate it as the Lagrangian dual problem
