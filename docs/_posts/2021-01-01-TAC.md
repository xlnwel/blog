...
Setting the first-order derivative of \\(\mathcal L\\) w.r.t. \\(\pi(a_t|s_t)\\) to zero, we have
...
where the last step is feasible as \\(\nu(a|s)\\) is zero for actions with positive probabilities; for any zero-probability action, (hmmm, not today.)

Also note that the second-order derivative of \\(\mathcal L\\) w.r.t. \\(\pi(a_t|s_t)\\) is alway negative:
...
where \\(|\mathcal A|\\) is the cardinality of \\(\mathcal A\\) and \\(q > 0\\).
