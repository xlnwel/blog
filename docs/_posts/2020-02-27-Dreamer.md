...
Acute readers may have noticed that Equation $$(4)$$ is, in fact, the variational evidence lower bound(ELBO) commonly seen in [VAE]({{ site.baseurl }}{% post_url 2018-08-01-VAE %}). Indeed, we can regard the dynamics model as a sequential VAE, where the representation model is the encoder, and the observation and reward models are the decoders.
