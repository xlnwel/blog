...
- **Dynamics learning:** Dreamer learns a dynamics model comprised of four components: a representation model \\(p(s_t|s_{t-1},a_{t-1},o_t)\\), a transition model \\(q(s_t|s_{t-1}, a_{t-1})\\), an observation model \\(q(o_t|s_t)\\), and a reward model \\(q(r_t|s_t)\\).
- **Behavior learning:** Based on the dynamics model, Dreamer learns an actor-critic architecture, where the actor is denoted as \\(q_\phi(a_t|s_t)\\) and the value function is \\(v_\psi(s_t)\\).
- **Environment interaction:** Dreamer interacts with the environment using the actor \\(q(a_t|s_t)\\) for data collection.
...
&= r_\tau+(1-\lambda)\gamma v_\psi(s_{\tau+1})+\lambda \gamma V_\lambda(s_{\tau+1}) \tag{5}\label{eq:5}
\end{align}
$$
