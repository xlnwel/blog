...
where \\(KL_t=\log{\pi(a|s)\over\pi_0(a|s)}\\), in which \\(\pi\\) is the policy we aim to optimize, \\(\pi_0\\) is a policy prior. One may also make the KL term action independent, i.e., \\(KL_t=D_{KL}(\pi(\cdot|s_t)\Vert\pi_0(\cdot|s_t))\\), the advantage of which is that we always ensures the non-negativity of \\(KL_t\\).
...
We now derive the optimal policy for Equation \\(\eqref{eq:1}\\), \\(\pi^*(a_t|s_t)\\) as follows
