...
**RL loss:** As the agent is trained with sequential data, Puigdomènech Badia et al. use a hybrid of [Retrace($$\lambda$$)]({{ site.baseurl }}{% post_url 2020-10-01-Retrace %}) and the [transformed Bellman operator]({{ site.baseurl }}{% post_url 2020-01-01-Apex-DQfQ %}). This gives us the following transformed Retrace operator

$$
\mathcal T^hQ(s_t,a_t):=h\left(h^{-1}(Q(s_t,a_t))+\sum_{k=t}^{t+n-1}\gamma^{k-t}\left(\prod_{i=t+1}^k c_i\right)\Big(r_k+\gamma\mathbb E_{\pi}h^{-1}(Q(s_{k+1},\cdot))-h^{-1}(Q(s_k,a_k))\Big)\right)\tag {1}
$$
