...
$$
\begin{align}
Q^\pi=\mathcal RQ^\pi(x_t,a_t)=\sum_{k\ge t}\gamma^{k-t}\mathbb E_\mu\left[\left(\prod_{i=t+1}^kc_i\right)\left(r_t+\gamma\big[\mathbb E_\pi Q^\pi(x_{k+1},\cdot)\big]-c_{k+1}Q^\pi(x_{k+1},a_{k+1})\right)\right]\tag{4}\label{eq:4}
\end{align}
$$
