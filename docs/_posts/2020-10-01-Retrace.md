...
$$
\begin{align}
\mathcal RQ(x_t,a_t)-Q^\pi(x_t,a_t)=\mathbb E_\mu\left[\sum_{k\ge t+1}\gamma^{k-t}\left(\prod_{i=t+1}^{t-1}c_i\right)\left(\mathbb E_\pi(Q-Q^\pi)(x_k,\cdot)-c_k(Q-Q^\pi)(x_k,a_k)\right)\right]\tag {2}
\end{align}
$$
...
$$
\mathcal RQ(x_t,a_t)=\sum_{k\ge t}\gamma^{k-t}\mathbb E_\mu\left[\left(\prod_{i=t+1}^kc_i\right)\left(r_t+\gamma\big[\mathbb E_\pi Q(x_{k+1},\cdot)\big]-c_{k+1}Q(x_{k+1},a_{k+1})\right)\right]\tag {3}
$$
...
With help of Equation $$(5)$$, we rewrite the state-based Retrace operator as follow
