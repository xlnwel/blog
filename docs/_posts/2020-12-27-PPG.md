...
where \\(r_t(\theta)={\pi_\theta(a_t|s_t)\over \pi_{\theta_{old}}(a_t|s_t)}\\), \\(\hat A_t\\) and \\(\hat V_t\\) are computed with GAE, \\(\theta\\) and \\(\phi\\) are the parameters of the policy and value networks, respectively. The use of disjoint parameters ensures two objectives do not interfere each other.
