...
&=\sum_{t=1}^T\mathbb E_{s_{t}, a_{t}\sim q}\left[r(s_t,a_t)+\mathcal H(q(a_t|s_t))\right]\quad\text{given }p(a_t|s_t) \text{ is uniform}\tag {4}
\end{align}
$$

In the penultimate step, we replace $$p(O_t\vert s_t,a_t)$$ with $$\exp(r(s_t,a_t))$$ as we stated in the [previous post]({{ site.baseurl }}{% post_url 2019-01-14-PGM %}). The last equation is obtained when we assume $$p(a_t)$$ is uniform and ignore it in the optimization process. Notice that the only difference between Eq.$$(3)$$ and Eq.$$(4)$$ is that Eq.$$(3)$$ adds $$\log p(a_t)$$ to reward $$r(s_t,a_t)$$ at each time step. In the following discussion, we focus on Eq.$$(4)$$, but the same analysis works for Eq.$$(3)$$ by adding $$\log p(a_t)$$ to the reward. The same process works if we just minimize the KL divergence between $$q(s_{1:T},a_{1:T})$$ and $$p(s_{1:T}, a_{1:T}\vert O_{1:T})$$.

Eq.$$(4)$$ is sometimes referred to as maximum entropy reinforcement learning since it maximizes the policy entropy as well as the conventional policy objective in reinforcement learning. Optimizing it eventually provides us a soft optimal policy (It's "soft" because $$p(s_{1:T}, a_{1:T}\vert O_{1:T})$$ is a probabilistic model). The entropy term is helpful in that it helps to mitigate the model collapse problem, meanwhile, providing some extra bonus for exploration.
