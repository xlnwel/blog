...
where the first term measures the reconstruction likelihood of generating \\( x \\) given \\( z \\) sampled from the approximate likelihood \\( q_{\phi}(z|x) \\), the output of the encoder network; the second term, acting as a regularizer, measures the difference between \\( q_\phi (z|x) \\) and \\( p_\theta (z) \\). It encourages the encoder output \\( q_\phi(z|x^{(i)}) \\) to be close to \\( p_\theta(z) \\). For a detailed derivation and explanation, please refer to my [previous post]({{ site.baseurl }}{% post_url 2018-08-07-VAE %}).
...
where \\( \beta>1 \\). Since the latent representation sampled from \\( p_\theta(z) \\), which is generally defined as the standard norm distribution, is naturally independent and factorial. By enforcing \\( q_\phi(z|x) \\) to be close to \\( p_\theta(z) \\), we implicitly impose disentanglement on the latent representation sampled from \\( q_\phi(z|x) \\).
...
- Term \\( 1 \\) intuitively maximizes the identifiability of the value \\( z \\) that generates \\( x \\). In other word, it enhances the connection between \\( x \\) and its corresponding \\( z \\) in the *decoder network*, and therefore we could anticipate that given \\( z\sim q_\phi(z|x_i) \\), then the reconstruction likelihood \\( p_\theta(x_i|z) \\) under the decoder network should be higher than the marginal likelihood \\( p_\theta(x_i) \\), which is averaged over \\( z\sim p_\theta(z) \\). Note that this term is intractable, since \\( p_\theta(x) \\) requires integral over \\( z \\).
...
Let \\( B_M={x_1, \dots, x_M} \\) be a minibatch of \\( M \\) indices where each element is sampled i.i.d. from \\( p(x) \\), so for any sampled batch instance \\( B_M \\), \\( p(B_M)={1\over N}^M \\). Let \\( r(B_M|x) \\) denote the probability of a sampled minibatch where one of the elements is fixed to be \\( x \\) and the rest are sampled i.i.d. from \\( p(x) \\). This gives \\( r(B_M|x)={1\over N}^{M-1} \\). Now we compute the expected marginal distribution \\( q(z) \\)  
...
- The last step follows from Jensen's inequality \\( E_{p(x)}[\log q(z|x)]\le \log E_{p(x)}[q(z|x)] \\)
