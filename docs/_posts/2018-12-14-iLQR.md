...
The model we have used so far is deterministic, where \\( s_{t+1}=f(s_t,a_t) \\). Things will become more complex if the model is stochastic, in which the next state is sampled from a probabilistic distribution, i.e., \\( s_{t+1}\sim p(s_{t+1}|s_t,a_t) \\). Fortunately, if the probabilistic distribution is a linear-Gaussian distribution with a constant covariance as follows, we could use the same algorithm as before.
