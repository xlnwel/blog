...
$$
\min_{\theta,\phi,\psi}\underbrace{\mathbb E\Big[\Vert \phi(s')-(\phi(s)+\psi(a)+e(s,a))\Vert_2^2+\lambda_{error}\Vert e(s,a)\Vert_2^2\Big]}_{Lagrangian\ of\ Eq.(3)}+\lambda_{info}\underbrace{\mathcal L_{info}}_{info\ loss}\tag{4}
$$

where the first term is obtained by applying the [Lagrangian function](https://en.wikipedia.org/wiki/Lagrange_multiplier) to the constraint optimization problem defined in Eq.$$(3)$$ and $$\mathcal L_{info}$$ denotes the negative of the mutual information objectives defined in Eq.$$(1)$$ and Eq.$$(2)$$.
