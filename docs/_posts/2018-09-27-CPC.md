...
where \\(p(d=i|X,c_t)\\) is the probability with \\([d=i]\\) being the indicator that sample \\(x^i\\) is the 'positive' sample drawn from \\(p(x_{t+k}|c_t)\\). The optimal probability for this loss is
...
As we can see, the optimal value for \\(f(x_{t+k}, c_t)\\) in Eq.2 is proportional to \\(p(x_{t+k}|c_t)\over p(x_{t+k})\\) and is independent of the choice of the number of negative samples \\(N-1\\).
