...
$$
a^{k-1}=\underset a{\arg\max}\left[Q(s,a)+P(s,a)\cdot{\sqrt{\sum_bN(s,b)}\over1+N(s,a)}\left(c_1+\log\left({\sum_b N(s,b)+c_2+1\over c_2}\right)\right)\right]\tag{1}
$$
...
In two-player zero-sum games, the value functions are assumed to be bounded within $$[0,1]$$(maybe a typo from the paper, I think it's supposed to be in the range of $$[-1, 1]$$. This is not important as the value becomes trivial when below $$0$$). This choice allows us to combine value estimates with probabilities using the pUCT rule (Equation $$(1)$$). However, since in many environments the value is unbounded, it is necessary to adjust the pUCT rule. To avoid environment specific knowledge, we replace the $$Q$$ value in Equation 1 with a normalized $$\bar Q$$ value computed as follows
