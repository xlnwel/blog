...
where \\(|J_f(X)|\\) is the [Jacobian determinant](https://en.wikipedia.org/wiki/Jacobian_matrix_and_determinant#Jacobian_determinant). This is consistent with the single variable case where we have
