...
We here justify the transformed Bellman operator; we show that the choice of \\(h(x)=\text{sign}(x)(\sqrt{|x|+1}-1)+\epsilon x\\) has the following property:

1. \\(h\\) is strictly monotonically increasing
2. \\(h\\) is Lipschitz continuous with Lipschitz constant \\(L_h={1\over 2}+\epsilon\\)
3. \\(h\\) is invertible with \\(h^{-1}(x)=\text{sign}(x)\left(\left({\sqrt{1+4\epsilon(|x|+1+\epsilon)}-1\over2\epsilon}\right)^2-1\right)\\)
...
**Lemma 1: **\\(h(x)=\text{sign}(x)(\sqrt{|x|+1}-1)+\epsilon x\\) is differentiable everywhere with derivative \\({d\over dx}h(x)={1\over 2\sqrt{|x|+1}}+\epsilon\\).

**Lemma 2:** \\(h^{-1}(x)=\text{sign}(x)\left(\left({\sqrt{1+4\epsilon(|x|+1+\epsilon)}-1\over2\epsilon}\right)^2-1\right)\\) is differentiable everywhere with derivative \\({d\over dx}h^{-1}(x)={1\over\epsilon}\left(1-{1\over\sqrt{1+4\epsilon(|x|+1+\epsilon)}}\right)\\).

We now prove all statements individually

1. We can prove that \\({d\over dx}h(x)={1\over{2\sqrt{|x|+1}}}+\epsilon>0\\) for all \\(x\in\mathbb R\\). Therefore, \\(h\\) is strictly monotonically increasing
...
4. Because \\({d\over dx}h^{-1}(x)={1\over\epsilon}\left(1-{1\over\sqrt{1+4\epsilon(|x|+1+\epsilon)}}\right)>0\\) for all \\(x\in\mathbb R\\), \\(h^{-1}\\) is strictly monotonically increasing
