...
We resorts to the following geometric interpretation to show that either \\((1)\\) or \\((2)\\) is true.
...
If case \\((1)\\) fails to hold, there is no \\(\pmb x^+,\pmb x^-\in\mathbb R^n\\) and \\(\pmb s\in\mathbb R^m\\) such that \\(\begin{bmatrix}\pmb x^+\\\\pmb x^-\\\\pmb s\end{bmatrix}\ge \pmb 0\\)
