...
Farkas' lemma equally says that either \\(\pmb b\\) in \\(C(\pmb A)\\) or there exists a hyperplane \\(P=\{\pmb x|\pmb y^\top \pmb x=0\}\\) that separates \\(\pmb b\\) and \\(C(\pmb A)\\). If \\(\pmb b\\) is not in \\(C(\pmb A)\\), then we can compute \\(\pmb y\\) as the vector orthogonal to the hyperplane. The following figure visualizes these two cases
