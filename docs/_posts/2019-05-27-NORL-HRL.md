...
Eq.$$(1)$$ gives us a glimpse at what the desired low-level policy looks like, but it does not offer any insight about how the representation learning function $$f$$ should be defined. Claim 4 in the paper shows that if we define $$\Psi$$ as a slight modification of the traditional objective given in Eq.$$(1)$$, then we may translate sub-optimality of $$\Psi$$ to a practical representation learning objective for $$f$$. (Do not freak out by the notations and terminologies, explanations will come shortly)
...
where we omit terms irrelevant to $$\theta$$ whenever possible. Note that the content in Eq.$$(7)$$ is exactly the objective of [MINE]({{ site.baseurl }}{% post_url 2018-09-01-MINE %}) estimator. This suggests our representation learning objective is in fact maximizing the mutual information between $$\pi$$ and $$s_{t+k}$$, discounting as $$k$$ increases. As in MINE, the gradient of the second term in brackets will introduce bias, the authors propose to replace the second term with
...
Since Eq.$$(7)$$ is essentially maximizing the mutual information between $$\pi$$ and $$s_{t+k}$$ for $$k\in[1, c]$$, other methods such as [DIM]({{ site.baseurl }}{% post_url 2018-09-09-DIM %}) may be further used to improve the performance.

###  Low-Level Policy Learning

Eq.$$(3)$$ suggests optimizing a policy $$\pi_{s_t,g}(a\vert s_{t+k-1},k)$$ for every $$s_t,g$$. This is equivalent to maximizing the parameterization $$\pi(a\vert s_t, g,s_{t+k},k)$$, a standard goal-conditioned hierarchical design. Standard RL algorithms may be employed to maximize the low-level reward implied by 
...
&=w_k\Big(-D(f(s_{t+k}), g)-\big(-D(f(s_{t+k}),\varphi(s_{t},\pi))-\log \mathbb E_{\tilde s\sim\rho(s_{t+k})}[\exp(-D(f_\theta(s_{t+k}),\varphi_\theta(s_t,\pi))]\big)\Big)\tag {9}
\end{align}
$$

where $$\pi$$ is approximated by $$a_{t:t+c-1}$$ as before. Now that we have the low-level reward computable from Eq.$$(9)$$, the low-level policy can be learned as we did in HIRO using some off-policy method. (Wait, [off-policy?](#llp))
...
As indicated by Eqs.$$(4)$$ and $$(6)$$, both low-level policy optimization and representation learning essentially minimize the KL divergence as below if we neglect all weights and discounted factors
