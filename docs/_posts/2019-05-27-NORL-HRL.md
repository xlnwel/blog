...
Since Eq.$$\eqref{eq:7}$$ is essentially maximizing the mutual information between $$\pi$$ and $$s_{t+k}$$ for $$k\in[1, c]$$, other methods such as [DIM]({{ site.baseurl }}{% post_url 2018-09-07-DIM %}) may be further used to improve the performance.
