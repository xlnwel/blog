...
Define $$V(s_t)=\mathbb E_{q}[\sum_{t'\ge t}\gamma^{t'-t}\big(r(s_{t'},a_{t'})-\alpha D_{KL}(q(a_{t'}\vert s_{t'})\Vert p(a_{t'}\vert s_{t'}))\big)]$$ and $$Q(s_t,a_t)=r(s_t,a_t)+\gamma\mathbb E_{s_{t+1}\sim p(s_{t+1}\vert s_t,a_t)}[V(s_{t+1})]$$. We can rewrite Equation $$(1)$$ as
...
Notice that the optimal solution of $$q$$ for Equation $$(2)$$ is $$q(a\vert s)\sim p(a\vert s)\exp(Q(s,a)/\alpha)$$. Therefore, we compute a sample-based policy $$q(a_i\vert s_i)={p(a_i\vert s_i)\exp(Q(s_i,a_i))\over\sum_jp(a_j\vert s_j)\exp(Q(s_j,a_j))}$$. This enables us to optimize $$p$$ by minimizing the KL divergence between $$q$$ and $$p$$

$$
p=\arg\min_pD_{KL}(q\Vert p)=\arg\max_p{1\over N}\sum_{s\sim\mathcal D}\sum_{a\sim q(a|s)}q(a|s)\log  p(a|s)\tag 4
$$

Unfortunately, sample based maximum likelihood can suffer from overfitting to samples. Additionally, $$q(a\vert s)$$ is unreliable due to a poor approximation of $$Q$$ -- potentially resulting in a large change of the action distribution in the wrong direction when optimizing Equation $$(4)$$. One effective regularization that address both concerns is to limit the overall change in $$q$$. This is done by adding an additional KL constraint and changing the objective from Equation $$(4)$$ to
...
We solve the objective from Equation $$(6)$$ by iteratively optimizing $$p$$ and $$\lambda$$ independently.

Moreover, [Abdolmaleki et al. 2018ab](#ref2) find it empirically better to replace the soft KL regularization in Equation $$(2)$$ with a hard constraint
...
Now we plug in Equation $$(9)$$ to the log likelihood ratio of the Lagrangian and it results in 
