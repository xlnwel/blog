...
where \\(\alpha=5\\) and \\(\beta=5\\) is used in the experiments. The scaling factor \\(T\over\sqrt{|\mathcal S|+|\mathcal A|}\\) acts as a normalizer on the dimensionality of the state and action spaces and on the time horizon of the task.
...
Note the runtime of a single reward step computation is \\(\mathcal O((|\mathcal S|+|\mathcal A|)D+{D^2\over T})\\), where \\(\mathcal O((|\mathcal S|+|\mathcal A|)D)\\) is used for computing all \\(d((s,a),(s^e,a^2))\\) at once and \\(\mathcal O({D^2\over T})\\) is for the while loop.
