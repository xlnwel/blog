...
excerpt: "We investigate the behavior of PPO and introduce new methods that forces the trust region constraint."
categories:
...
1. If PPO bounds the likelihood ratio \\(r(\pi)={\pi(a|s)\over\pi_{old}(a|s)}\\)?
...
They find the KL divergence constraint usually results in more exploration than the ratio constraint. One explanation is that, in the ratio constraint, the action with larger likelihood updates more often than the others -- because \\(\pi(a|s)\over\pi_{old}(a|s)\\) is less likely to exceed the bound when \\(\pi_{old}(a|s)\\) is large -- making the policy more deterministic and less explore. Figure 9 plots the policy entropy on four Mujoco tasks. We can see that trust region-based methods have higher entropy than ratio-based methods.
