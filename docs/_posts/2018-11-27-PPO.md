...
Let \\( f(\theta) \\) be the objective concave function to be maximized. At the \\( m \\) step of the MM algorithm, \\( m=0, 1… \\), the constructed function \\( g(\theta|\theta_m) \\) will be called the minorized version of the objective function (namely, the surrogate function) at \\( \theta_m \\) if 
...
By maximizing \\( g(\theta|\theta_m) \\) instead of \\( f(\theta) \\) and let
