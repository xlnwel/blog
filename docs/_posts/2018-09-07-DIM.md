...
Optimizing this objective, the fraction approximates the probability of \\(E_\psi(x)=y\\) for a specific \\(y\\) and some \\(x\\) drawn from \\(p(x)\\). We denote this optimal probability as \\(p(E_\psi(x)=y|X,y)\\), and apply Bayes' theorem
...
Thus, the optimal value of \\(f(x,y)\\) is given by \\(p(x|y)\over p(x)\\).
