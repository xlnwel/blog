...
$$
\begin{align}
I^{DV}(X; Y)&=D_{KL}(P(X,Y)\Vert P(X)P(Y))\\\
&=E_{p(x,y)}\left[T(x, y)\right]-\log E_{p(x')p(y')}\left[e^{T(x', y')}\right]
\end{align}
$$
...
$$
\begin{align}
I^{JSD}(X; Y)&:= D_{JS}(P(X,Y)\Vert P(X), P(Y))\\\
&\ge2\log 2+E_{p(x,y)}\left[-\mathrm{sp}(-T(x, y))\right]-E_{p(x')p(y)}\left[\mathrm{sp}(T(x', y))\right]\tag{1}\\\
&=2\log 2+E_{p(x,y)}\left[\log\sigma(T(x, y))\right]+E_{p(x')p(y)}\left[\log\sigma(-T(x', y))\right]\tag{2}\\\
\end{align}
$$
...
<a name="back1"></a>We prove Eq.\\((1)\\) and Eq.\\((2)\\) are the lower bound on \\(D_{JS}(P(X,Y)\Vert P(X), P(Y))\\) in [the end](#proof).
...
Due to distinct motivations, DV and InfoNCE adopt different resampling process: DV resamples all \\(x'\\) and \\(y'\\) from their marginal distributions in the negative log term while InfoNCE only resamples \\(x'\\). 
