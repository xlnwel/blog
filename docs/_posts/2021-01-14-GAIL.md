...
where we omit domains whenever it does not cause any ambiguity. Because IRL learned from a finite expert dataset can easily overfit, we introduce a convex regularizer: \\(\psi:\mathbb R^{\mathcal S\times\mathcal A}\rightarrow \mathbb R\\). The regularizer is assumed to be convex because we want learn a reward function that minimizes it. Now we define the regularized IRL objective
...
&\qquad\color{red}{D(s,a)={1\over1+e^{-\gamma(s,a)}}\text{, we omit all }(s,a)\text{ for simplicity}}\\\
=&\sum_{s,a}\max_\gamma\rho_\pi\log{1\over 1+e^{\gamma}}+\rho_E\log{1\over 1+e^{-\gamma}}\\\
&\qquad\color{red}{\phi=\log{1\over 1+e^{\gamma}}}\\\
=&\sum_{s,a}\max_\gamma\rho_\pi\phi(\gamma)+\rho_E\phi(-\gamma)\\\
=&\sum_{s,a}\max_\gamma\rho_\pi\phi(\gamma)+\rho_E\phi(-\phi^{-1}(\phi(\gamma)))\\\
&\qquad\color{red}{c=\phi(\gamma)}\\\
...
Here, the condition is introduced because \\(D=\exp(\phi)\in[0,1]\\). 
...
Therefore, \\(\phi(-\phi^{-1}(c))\\) is convex, and \\(g(c)\\) and \\(\psi(c)\\) is convex. 
