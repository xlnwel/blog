...
$$
   ...
   where \\(\mu(u|s)\\) is the diagonal Gaussian distribution produced by the neural network before squashing. Because the Jacobian \\(da/du=\mathrm{diag}(1-\tanh^2(u))\\) is diagonal, the log-likelihood has a simple form
