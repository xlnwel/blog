...
If the inner loop learning is repeated $$N$$ times, MAML only uses the final weights for outer loop learning. As we will see in the next post, this could be troublesome, causing unstable learning when $$N$$ is large.
