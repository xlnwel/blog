...
**Example.** For arbitrary matrix $$\pmb A$$, the second derivative of $$\pmb x^\top\pmb A\pmb x$$ is

$$
\begin{align}
{\partial\over\partial\pmb x}{\partial\over\partial\pmb x}\pmb x^\top\pmb A\pmb x
=&{\partial\over\partial\pmb x}({\partial\over\partial\pmb x}\pmb x^\top\pmb A\bar{\pmb x}+{\partial\over\partial\pmb x}\bar{\pmb x}^\top\pmb A\pmb x)\quad\color{red}{\bar{\pmb x}\text{ is treated as a constant}}\\\
=&{\partial\over\partial\pmb x}(\pmb x^\top\pmb A+\pmb x^\top\pmb A^\top)\\\
=&\pmb A+\pmb A^\top
\end{align}
$$
...
for all $$\pmb x,\pmb y\in \mathcal X$$ and all $$t\in[0,1]$$. The first-order Taylor expansion is valid as $$f$$ is convex and it's Hession, if exits, is positive semi-definite.
