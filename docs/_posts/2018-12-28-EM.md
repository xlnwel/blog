...
More often than not, we cannot model the data distribution \\(p(x)\\) with a given distribution as the data distribution is more complex than the given one. In this case, we introduce some latent variable \\(z\\) and assume \\(p(x|z)\\) follows that distribution. In other words, we regards the data distribution \\(p(x)\\) as a combination of several independent distributions \\(p(x|z)\\) whose weights are given by \\(p(z)\\), i.e., \\(p(x)=\sum_zp(z)p(x|z)\\). In practice, we usually maximize the log likelihood for numeric stability and parameterizes the distribution by \\(\theta\\) as follows
...
Intuitively, we could see from Eq.\\( (2) \\) that if the KL term is zero, then maximizing the objective defined in Eq.\\( (1) \\) is equivalent to maximizing the first term in Eq.\\( (2) \\) since \\( H(Z) \\) is constant w.r.t. \\( \theta \\). In other words, we may solve Eq.\\( (1) \\) by making \\( q(z)=p(z|x;\theta) \\) and then maximizing \\( \mathbb E_{z\sim q(z)}\left[\log{p(x,z;\theta)}\right] \\). The second step, however, changes \\( \theta \\), making \\( q(z) \ne p(z|x; \theta) \\), so we reassign \\( q(z) \\) and repeat the above process until convergence, which is exaatly what the EM algorithm does.

Summing it up, the EM algorithm iterates the following two steps:

1. **E-step**: Evaluate the conditional distribution over \\( z \\) given the dataset and the current model parameters \\( p(z|x;\theta^{(t)}) \\), then we have \\( Q(\theta, \theta^{(t)}):=\mathbb E_{z\sim p(z|x;\theta^{(t)})}[\log p(x,z;\theta)] \\)
2. **M-step**: Maximize \\( Q(\theta, \theta^{(t)})\\), \\( \theta^{(t+1)}\leftarrow \arg\max_\theta Q(\theta,\theta^{(t)}) \\)

<a name="back1"></a>A detailed proof that this process actually works is given in [the end](#proof1).
...
By setting \\( \theta=\theta^{(t+1)} \\) and \\( q(z)=p(z|x;\theta^{(t)}) \\), we get
...
where the last inequality comes by the definition of the M-step. Let \\( p(\theta)=p(z|x;\theta) \\), then we have
...
In the E-step, we first compute \\( p(z|x;\theta^{(t)}) \\), i.e., all \\( r_{i,k} \\) at time step \\(t\\), from Bayes' theorem. Then we compute \\( Q(\theta, \theta^{(t)}) \\) as follows
