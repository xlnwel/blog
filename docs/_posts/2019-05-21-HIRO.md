...
It seems to me that the guarantee is only carried out by the goal transition model $$h$$ defined in Eq.$$(2)$$ and low-level reward $$r$$ defined in Eq.$$(1)$$. However, there seems no guarantee from the high-level policy which is exactly the part producing the goal! No idea if this will be an issue.
