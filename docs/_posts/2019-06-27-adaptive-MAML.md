...
Now let's consider a task changing dynamically due to non-stationary of the environment. We address this problem using a temporal probabilistic graphical model \\((b)\\), where \\(T_{i}\\) and \\(T_{i+1}\\) are sequentially dependent on transition distribution \\(P(T_{i+1}|T_i)\\). To adapt for task shift, we aim to learn policy parameter \\(\theta\\) and meta-learning rate parameter \\(\alpha\\) such that \\(\theta\\) can quickly adapt to task \\(T_{i+1}\\) from \\(\tau_{i}^{1:K}\\) within few gradient steps with step size \\(\alpha\\). The following figure illustrates this process.
